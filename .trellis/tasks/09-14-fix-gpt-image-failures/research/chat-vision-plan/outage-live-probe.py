"""Corrective live probe under the unchanged production systemd sandbox.

Derived from live-vision-probe.py; synthetic records/cards only, production chat
feature flags, no QQ client/generation/production DB writes. Results on stdout.
"""
import asyncio
...
async def main():
    ...
    with tempfile.TemporaryDirectory(prefix='aiqq-outage-vision-', dir='/var/lib/aiqq') as directory:
        root = Path(directory)
        ...
        try:
            ...
            agent = ChatAgent(backend, system_prompt='准确读取图片，使用简洁中文回答。', web_search_enabled=config.ai.web_search_enabled,
                              gpt_image_skill_enabled=config.ai.gpt_image_skill_enabled,
    ...
    result['production_feature_flags'] = {'web_search': config.ai.web_search_enabled, 'gpt_image_skill': config.ai.gpt_image_skill_enabled}
    print(json.dumps(result, ensure_ascii=False))
    if not result['success'] or result.get('generation_requested', False):
        raise SystemExit(1)
